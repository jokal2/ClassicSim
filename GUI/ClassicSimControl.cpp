...
ClassicSimControl::ClassicSimControl(QObject* parent) :
    ...
    QObject::connect(thread_pool, &SimulationThreadPool::threads_finished, this, &ClassicSimControl::run_next_step);
...
ScaleResultModel* ClassicSimControl::get_scale_result_model() const {
...
void ClassicSimControl::update_displayed_dps_value(const double new_dps_value) {
...
void ClassicSimControl::update_displayed_tps_value(const double new_tps_value) {
...
void ClassicSimControl::update_displayed_raid_tps_value(const double new_tps_value) {
...
void ClassicSimControl::calculate_displayed_tps_value() {
    // Note: intended for local testing to build confidence in thread results
...
void static setSetup(ClassicSimControl* g, Setup setup) {}

void ClassicSimControl::run_next_step() {
    puts("HI");
}

void ClassicSimControl::setMyJunk() {
    auto e = current_char->get_equipment();

    e->set_mainhand(Items::DragonfangBlade);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_offhand(Items::CoreHoundTooth);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_ranged(Items::BlastershotLauncher);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_head(Items::LionheartHelm);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_neck(Items::OnyxiaToothPendant);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_back(Items::DragonsBloodCape);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_chest(Items::BreastplateofMight);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_wrist(Items::WristguardsofTrueFlight);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_gloves(Items::AgedCoreLeatherGloves);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_belt(Items::OnslaughtGirdle);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_legs(Items::LegplatesofWrath);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_boots(Items::SabatonsofWrath);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_ring1(Items::BandofAccuria);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_ring2(Items::QuickStrikeRing);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_trinket1(Items::BlackhandsBreadth);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
    e->set_trinket2(Items::DiamondFlask);
    emit equipmentChanged();
    emit statsChanged();
    emit enchantChanged();
